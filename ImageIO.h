...
// nsJPEGDecoder (see image/decoders/nsJPEGDecoder.cpp).
//
// Given the full JPEG image dimensions and the final target dimensions, returns
// 1 (no pre-scaling), 2, 4, or 8.
inline int ComputeIdctScaleDenom(int aImageWidth, int aImageHeight,
                                 int aTargetWidth, int aTargetHeight) {
  uint32_t widthRatio =
      static_cast<uint32_t>(aImageWidth) / static_cast<uint32_t>(aTargetWidth);
  uint32_t heightRatio = static_cast<uint32_t>(aImageHeight) /
                         static_cast<uint32_t>(aTargetHeight);
  uint32_t minRatio = widthRatio < heightRatio ? widthRatio : heightRatio;
